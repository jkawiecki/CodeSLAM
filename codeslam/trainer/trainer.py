    ...
    try:
        for epoch in range(start_epoch, epochs):
            ...
            for iteration, (images, targets) in enumerate(data_loader, start_iter):
                ...
                # Weighting
                weights = dict(kl_div=weighting.beta(global_step, milestone)) #, uncertaintyaware_loss=1 if epochs//2>20 else 0)
                ...
                if cfg.TRAINER.EVAL_STEP > 0 and global_step % cfg.TRAINER.EVAL_STEP == 0:
                    model.eval()
                    eval_results = inference.do_evaluation(cfg, model, iteration=global_step)
                    model.train()
